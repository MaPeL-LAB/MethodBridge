...
Earlier commits described deterministic canned responses as an empirical
five-model bake-off and reported unsupported candidate, throughput, memory,
perplexity, retention, and quantization conclusions. Those statements remain
withdrawn.

The retained engineering work is useful: candidate and upstream registries, GGUF
conversion and quantization paths, deterministic prompt routing, frozen benchmark,
private challenger boundary, hardware validation, constrained simulation, and
release packaging scaffolding. None establishes a real winner.
...
`Qwen/Qwen3-1.7B` with `Q5_K_M` is only a documentary first-test hypothesis.
It is not a finalist, winner, optimized choice, or approved submission artifact.

## Pre-local controls now implemented

- machine-validatable campaign identity, candidate order, runtime settings, and stopping conditions;
- explicit eligibility and human execution authorization fields;
- shareable real-run evidence schema with no raw text or local paths;
- qualified semantic adjudication schema;
- public claims validator covering report, model card, demo, and Devpost surfaces;
- separate human-controlled release authorization;
...
- finalize report, model card, metadata, architecture, video, and submission.

No model, accuracy, performance, memory, thermal, retention, quantization, or
release claim is established until its evidence class and human gate are met.
